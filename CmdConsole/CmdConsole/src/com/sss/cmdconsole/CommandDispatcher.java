...
public class CommandDispatcher implements Runnable, IStdOut
{
	 ...
	 public void run()
	 {
		...
		fileSys.setCurDir(CFunc.getAppDir());
...
//					    	File f = new File(CFunc.getAppDir());
//					    	fileSys.listDir(f);
					    }
					 }
				  } 
...
//			    	 fileSys.listDir(f); 
			...
			else if (builtInCmd == CmdInfo.BuiltInCmd.touch) {
				   fileSys.printCurDir();
//					writeln(CFunc.getAppDir());
				}
...
//				writeln(CFunc.getAppDir());
			...
			else if (builtInCmd == CmdInfo.BuiltInCmd.ps) {
				writeln("");
				writeln(CFunc.getString(R.string.user_id) + " [" + CFunc.getUserID() + "]");
				writeln("");
			}
