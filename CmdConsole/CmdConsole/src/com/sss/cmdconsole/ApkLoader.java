...
public class ApkLoader 
{
   ...
   /**
    * Delete the cached dex file. Since if a new apk is to be run but a stale cached dex
    * already exists, this console app will be signaled to die unconditionally. 
    * 
    * @param apkPath -- the apk file path ready to run
    */
   private static void deleteCachedDex(String apkPath)
   {
	  ...
	  String extension = (dotPos >= 0 && dotPos != fileName.length() - 1) ? fileName.substring(dotPos + 1) : null;
   ...
   /**
    * Read the class name with 'main' method declared, which should be located at ENTRYPOINT_DECLARATION
    * inside the archive.
    * @param apkPath
    * @return
    * @throws IOException
    */
   private static String readEntryPointClass(String apkPath) throws IOException
   {
	  ...
	  if (zipEntry == null) {
		 apkFile.close();
	     throw new IOException("Entry point declaration file does not exist -- " + ENTRYPOINT_DECLARATION);
	  }
	  
	  InputStream inEntry = apkFile.getInputStream(zipEntry);
	  BufferedReader reader = new BufferedReader(new InputStreamReader(inEntry));
