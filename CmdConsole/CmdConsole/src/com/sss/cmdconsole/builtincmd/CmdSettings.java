...
public class CmdSettings 
{
   ...
   public void setValue(Setting setting, Object val)
   {
	  ...
	  if (val instanceof Boolean)
		  ...
		  saveVal = String.valueOf((Integer)val);
	  else if (val instanceof Long)
		  saveVal = String.valueOf((Long)val);
   ...
   public void setValue(String name, String value)
   {
	  ...
	  if (dbInst.update(TBL_SETTING, values, DBName.s_name.name() + "='" + name + "'", null) != 1) {
   ...
   {
	   String val = null;
       Cursor c = dbInst.query(TBL_SETTING, new String[]{DBName.s_value.name()}, DBName.s_name.name() + "='" + name + "'",
                               null, null, null, null);
   ...
   {
	   String val = null;
       Cursor c = dbInst.query(TBL_CMD_HISTORY, new String[]{DBName.cmd_string.name()}, DBName.history_id.name() + "=" + id,
                               null, null, null, null);
       if (c != null && c.moveToFirst()) {
          val = c.getString(0);
       }
   ...
   public LinkedHashMap<Integer, String> getHistoryList(int nrItems)
   {
	   ...
	   int startID = (nrItems > 0) ? (maxID - nrItems) : (maxID - DEFAULT_HISTORY_NUM);
	   
	   Cursor c = dbInst.query(TBL_CMD_HISTORY, null, DBName.history_id.name() + ">" + startID,
                        null, null, null, DBName.history_id.name() + " ASC");
   ...
   {
	   Cursor c = dbInst.rawQuery("select max(" + DBName.history_id.name() + ") from " + TBL_CMD_HISTORY, null);
	   int maxID = 0;
	   if (c != null && c.moveToFirst()) {
		  maxID = c.getInt(0);
	   }
   ...
   {
	   Cursor c = dbInst.rawQuery("select min(" + DBName.history_id.name() + ") from " + TBL_CMD_HISTORY, null);
	   int minID = 0;
	   if (c != null && c.moveToFirst()) {
		  minID = c.getInt(0);
	   }
    ...
    * @param idAndLess -- 0 if to clear all
    */
   public void clearHistory(int idAndLess)
   {
	   if (idAndLess < 0) {
		   ...
		   dbInst.delete(TBL_CMD_HISTORY, DBName.history_id.name() + "<=" + idAndLess, null);
		   int minID = getMinCmdID();
		   if (minID > 1) {
			   // update remaining IDs to start from 1
			   int offset = minID - 1;
			   dbInst.execSQL("update " + TBL_CMD_HISTORY + " set " + DBName.history_id.name() + " = " + DBName.history_id.name() + " - " + offset);
		   }
	   }
   }
}
